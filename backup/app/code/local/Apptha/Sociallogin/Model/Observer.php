...
class Apptha_Sociallogin_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    public function checkcaptchaforcreate($observer) {
        $formId = 'user_create';
        $captchaModelUserCreate = Mage::helper ( 'captcha' )->getCaptcha ( $formId );
        if ($captchaModelUserCreate->isRequired ()) {
            $controller = $observer->getControllerAction ();
            $word = $this->_getCaptchaString ( $controller->getRequest (), $formId );
            if (! $captchaModelUserCreate->isCorrect ( $word )) {
    ...
    /**
     * Function to check captcha for forget password
     *
     * @param unknown $observer
     * @return object
     */
    public function checkCaptchaforgetpassword($observer) {
        ...
        $captchaModelForgotPassword = Mage::helper ( 'captcha' )->getCaptcha ( $formId );
        if ($captchaModelForgotPassword->isRequired ()) {
            $controller = $observer->getControllerAction ();
            $word = $this->_getCaptchaString ( $controller->getRequest (), $formId );
            if (! $captchaModelForgotPassword->isCorrect ( $word )) {
