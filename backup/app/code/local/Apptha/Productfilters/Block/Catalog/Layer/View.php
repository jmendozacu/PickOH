...
class Apptha_Productfilters_Block_Catalog_Layer_View extends Mage_Catalog_Block_Layer_View{
     ...
     * {@inheritDoc}
     * @see Mage_Catalog_Block_Layer_View::_prepareLayout()
     */
    protected function _prepareLayout() {
        if (! $this->helper ( 'productfilters' )->isEnabled ()) {
            return parent::_prepareLayout ();
        }
        $specialBlock = $this->getLayout ()->createBlock ( $this->_specialFilterBlockName )->setLayer ( $this->getLayer () )->init ();
        $starRateBlock = $this->getLayout ()->createBlock ( $this->_starRatingFilterBlockName )->setLayer ( $this->getLayer () )->init ();
     ...
     * {@inheritDoc}
     * @see Mage_Catalog_Block_Layer_View::getFilters()
     * @return array
     */
    public function getFilters() {
        ...
        if (($specialFilter = $this->_getSpecialFilter ())) {
           $filters = array_merge ( array_slice ( $filters, 0, static::SPECIAL_FILTER_POSITION - 1 ), array (
                    $specialFilter
            ), array_slice ( $filters, static::SPECIAL_FILTER_POSITION - 1, count ( $filters ) - 1 ) );
        }

        if (($starRateBlock = $this->_getStarFilter ())) {
            $filters = array_merge ( array_slice ( $filters, 0, static::STARRATE_FILTER_POSITION - 1 ), array (
                    $starRateBlock
            ), array_slice ( $filters, static::STARRATE_FILTER_POSITION - 1, count ( $filters ) - 1 ) );
        }
