...
class Apptha_Productfilters_Helper_Data extends Mage_Core_Helper_Data{
    ...
    /**
     * Function to get Category Parent Id
     *
     * @param unknown $id
     * @return int
     */
    ...
    /**
     *
     * Function to get tree categories
     * @param unknown $parentId
     * @param unknown $productcollection
     * @param unknown $isChild
     * @return string
     */
    public function getTreeCategories($parentId, $productcollection, $isChild) {
        $allCats = Mage::getModel ( 'catalog/category' )->getCollection ()->addAttributeToSelect ( '*' )->addAttributeToFilter ( 'is_active', '1' )->addAttributeToFilter ( 'include_in_menu', '1' )->addAttributeToFilter ( 'parent_id', array (
                'eq' => $parentId 
        ...
        foreach ( $allCats as $category ) {
            ...
            $_testproductCollection = Mage::getResourceModel ( 'catalog/product_collection' )->addAttributeToFilter ( 'entity_id', array (
                    'in' => $productIdsCollection 
            ...
            if (count ( $_testproductCollection ) <= 0) {
                ...
                $html .= '</span>';
                 ...
                 href="' . $category->getUrl () . '">' . $category->getName () . '</a>' . '<span>(' . count ( $_testproductCollection ) . ')</span>';
            }
            if (! empty ( $subhtml )) {
                $html .= $subhtml;
            }
