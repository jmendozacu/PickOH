...
class Apptha_Productfilters_Model_Catalog_Layer_Filter_Special extends Mage_Catalog_Model_Layer_Filter_Abstract{
    ...
    public function apply(Zend_Controller_Request_Abstract $request, $filterBlock) {
        ...
        if ($filter == static::FILTER_ON_NEW) {
            ...
            $this->getLayer ()->getProductCollection ()->addAttributeToFilter ( 'news_from_date', array (
                    'or' => array ( 0 => array ( 'date' => true, 'to' => $todayEndOfDayDate ),
        ...
        } else if ($filter == static::FILTER_ON_POPULAR) {
            $stateLabel = Mage::helper ( 'productfilters' )->__ ( 'Popular Products' );
            $id = array ();
            $products = Mage::getResourceModel ( 'reports/product_collection' )->addAttributeToSelect ( '*' )->setStoreId ( Mage::app ()->getStore ()->getId () )->addStoreFilter ( Mage::app ()->getStore ()->getId () )->addViewsCount ()->addCategoryFilter ( Mage::registry ( 'current_category' ) );
            foreach ( $products as $product ) {
                array_push ( $id, $product->getData ( 'entity_id' ) );
            }

            $ids = array_map ( 'intval', $id );
            $collection = $this->getLayer ()->getProductCollection ()->addAttributeToFilter ( 'entity_id', array (
                    'in' => $id
            ) )->getSelect ();
    ...
    protected function _getCount() {
        ...
        $popularProducts = Mage::getResourceModel ( 'reports/product_collection' )->addAttributeToSelect ( '*' )->setStoreId ( Mage::app ()->getStore ()->getId () )->addStoreFilter ( Mage::app ()->getStore ()->getId () )->addViewsCount ()->addCategoryFilter ( Mage::registry ( 'current_category' ) );
        ...
        $todayStartOfDayDate = Mage::app ()->getLocale ()->date ()->setTime ( '00:00:00' )->toString ( Varien_Date::DATETIME_INTERNAL_FORMAT );
        $todayEndOfDayDate = Mage::app ()->getLocale ()->date ()->setTime ( '23:59:59' )->toString ( Varien_Date::DATETIME_INTERNAL_FORMAT );
        $latestProducts = Mage::getResourceModel ( 'reports/product_collection' )->addAttributeToSelect ( '*' )->setStoreId ( Mage::app ()->getStore ()->getId () )->addStoreFilter ( Mage::app ()->getStore ()->getId () )->addCategoryFilter ( Mage::registry ( 'current_category' ) )->addAttributeToFilter ( 'news_from_date', array (
                'or' => array (
                        0 => array (
                                'date' => true,
