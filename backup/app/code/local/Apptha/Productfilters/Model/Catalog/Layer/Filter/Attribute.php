...
class Apptha_Productfilters_Model_Catalog_Layer_Filter_Attribute extends Mage_Catalog_Model_Layer_Filter_Attribute{
    ...
    protected function _getItemsData() {
        ...
        if ($data === null) {
            ...
            foreach ( $options as $option ) {
                $this->getOptionVale($option);
                if (Mage::helper ( 'core/string' )->strlen ( $option ['value'] )) {

                    if (($this->_getIsFilterableAttribute ( $attribute ) == static::OPTIONS_ONLY_WITH_RESULTS) && (! empty ( $optionsCount [$option ['value']] ))) {
                        $data [] = array (
                                    'label' => $option ['label'],
                                    'value' => $option ['value'],
    ...
    /**
     * Function to get Items Option Data
     * @param array $option
     * @return void
     */
    public function getOptionVale($option) {
