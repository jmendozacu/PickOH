...
class Apptha_Marketplace_Model_Observer {
    ...
    public function successAfter($observer) {
        ...
        $shippingPrice = $sellerShippingPrice = 0;
            ...
            if ($sellerId) {
                ...
                $sellerModel=Mage::getModel('marketplace/sellerprofile')->load($sellerId,'seller_id');
                $sellerDefaultCountry=$sellerModel->getCountry();
                ...
                if(Mage::getStoreConfig ( 'marketplace/shipping/shippingcost' ) && $shippingDescription == $marketplace_shipping_description ){
                    ...
                    $nationalShippingPrice = $sellerModel->getNationalShippingCost();
                    $internationalShippingPrice = $sellerModel->getInternationalShippingCost();
 ...
 $sellerStore = Mage::app ()->getStore ()->getName ();
		...
		$orderArray=array('order_id'=>$incrementId,'sellerSessionId'=>$id);
		Mage::getSingleton('checkout/session')->setCheckoutOrderData($orderArray);
		...
		$currencySymbol = Mage::app ()->getLocale ()->currency ( Mage::app ()->getStore ()->getCurrentCurrencyCode () )->getSymbol ();
                ...
                Mage::helper('marketplace/order')->sendSellerShippingData($sellerId,$customerFirstname,$customerEmail,$sellerStore,$incrementId,$marketplaceGroupData);
            }
          } else {
            foreach ( $sellerIds as $id ) {
		...
		$sellerProfile = Mage::getModel ( 'marketplace/sellerprofile' )->load ( $id, 'seller_id' );
	        ...
	        $currencySymbol = Mage::app ()->getLocale ()->currency ( Mage::app ()->getStore ()->getCurrentCurrencyCode () )->getSymbol ();
                ...
                Mage::helper('marketplace/order')->sendOrderEmailData($sellerId,$incrementId,$sellerStore,$customerEmail,$customerFirstname,$marketplaceData);
            }
         }
