...
class Apptha_Marketplace_Helper_Marketplace extends Mage_Core_Helper_Abstract {
    ...
    public function getAmountReceived() {
        ...
        $transactionCollection = Mage::getModel ( 'marketplace/transaction' )->getCollection ()->addFieldToSelect ( 'seller_commission' )->addFieldToFilter ( 'seller_id', $getSellerId )->addFieldToFilter ( 'paid', 1 );
    ...
    public function getAmountRemaining() {
        ...
        $collectionPaidUpto = Mage::getModel ( 'marketplace/transaction' )->getCollection ()->addFieldToSelect ( 'paid_upto' )->addFieldToFilter ( 'seller_id', $sellerId )->addFieldToFilter ( 'paid', 0 );
        $collectionPaidUpto->getSelect ()->columns ( 'SUM(paid_upto) AS paid_upto' )->group ( 'seller_id' );
    ...
    public function subscriptionEndDate($sellerId) {
        ...
        if ($subscribed->getId ()) {
            ...
            if ($subscriptionPeriod == 1 && $offerPeriod == 1) {
                $subscriptionPlanEndDate = date ( 'Y-m-d', strtotime ( '+' . $validityPeriod + $offerValidityPeriod . ' months', strtotime ( $subscriptionStartDate ) ) );
            } elseif ($subscriptionPeriod == 2 && $offerPeriod == 2) {
                $subscriptionPlanEndDate = date ( 'Y-m-d', strtotime ( '+' . $validityPeriod + $offerValidityPeriod . ' years', strtotime ( $subscriptionStartDate ) ) );
            } elseif ($subscriptionPeriod == 1 && $offerPeriod == 2) {
                $subscriptionPlanEndDate = date ( 'Y-m-d', strtotime ( '+' . $offerValidityPeriod . ' years ' . $validityPeriod . ' months', strtotime ( $subscriptionStartDate ) ) );
            } elseif ($subscriptionPeriod == 2 && $offerPeriod == 1) {
                $subscriptionPlanEndDate = date ( 'Y-m-d', strtotime ( '+' . $validityPeriod . ' years ' . $offerValidityPeriod . ' months', strtotime ( $subscriptionStartDate ) ) );
            } elseif ($subscriptionPeriod == 2 && $offerPeriod == 0) {
                $subscriptionPlanEndDate = date ( 'Y-m-d', strtotime ( '+' . $validityPeriod . ' years ', strtotime ( $subscriptionStartDate ) ) );
            } elseif ($subscriptionPeriod == 1 && $offerPeriod == 0) {
                $subscriptionPlanEndDate = date ( 'Y-m-d', strtotime ( '+' . $validityPeriod . ' months', strtotime ( $subscriptionStartDate ) ) );
            }
        }
        return $subscriptionPlanEndDate;
    }
    public function outofstock($productId) {
         ...
         * @var unknown
         */
        $assignProducts = Mage::getResourceModel ( 'catalog/product_collection' )->addAttributeToSelect ( array (
                'name'
        ) )->addAttributeToFilter ( 'assign_product_id', $productId )->load ();

        return  $assignProducts->getSize();
    }
