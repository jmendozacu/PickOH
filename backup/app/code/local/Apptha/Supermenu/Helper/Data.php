...
class Apptha_Supermenu_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getSuperMenuData() {
        ...
        $categories = $helper->getStoreCategories ( 'isvisible', false, true );
        if (is_object ( $categories )) {
            $categories = $helper->getStoreCategories ( 'isvisible', false );
        }
    ...
    public function getSuperMenuContent() {
        ...
        $homeLink = '';
        $homeLink .= '<div class="category_menu"><ul>';
        /**
         * Menu Content
         */
        ...
        $allCategories = $this->__ ( 'All Categories' );
        if ($i != 0) {
            $topMenu = '' . $homeLink . '' . $topMenuContent . '<li class="all_categories"><a href="' . Mage::getBaseUrl () . 'marketplace/general/allcategories' . '">' . $allCategories . '</a></li><div class="clearBoth"></div>';
        }
        return array (
                        'topMenu' => $topMenu,
    ...
    public function isMobile() {
        $regex_match = "/(nokia|ipad|iphone|android|motorola|^mot\-|softbank|foma|docomo|kddi|up\.browser|up\.link|" . "htc|dopod|blazer|netfront|helio|hosin|huawei|novarra|CoolPad|webos|techfaith|palmsource|" . "blackberry|alcatel|amoi|ktouch|nexian|samsung|^sam\-|s[cg]h|^lge|ericsson|philips|sagem|wellcom|bunjalloo|maui|" . "symbian|smartphone|mmp|midp|wap|phone|windows ce|iemobile|^spice|^bird|^zte\-|longcos|pantech|gionee|^sie\-|portalmmm|" . "jig\s browser|hiptop|^ucweb|^benq|haier|^lct|opera\s*mobi|opera\*mini|320x320|240x320|176x220" . ")/i";
        /**
         * Check the server profile
         * return boolean value
         */
        if ((strpos ( strtolower ( $_SERVER ['HTTP_ACCEPT'] ), 'application/vnd.wap.xhtml+xml' ) > 0) || ((isset ( $_SERVER ['HTTP_X_WAP_PROFILE'] ) || isset ( $_SERVER ['HTTP_PROFILE'] )))|| preg_match ( $regex_match, strtolower ( $_SERVER ['HTTP_USER_AGENT'] ) )) {
            return TRUE;
        }
        /**
         ...
         */
        if (isset ( $_SERVER ['ALL_HTTP'] ) && strpos ( strtolower ( $_SERVER ['ALL_HTTP'] ), 'OperaMini' ) > 0|| in_array ( $mobile_ua, $mobile_agents )) {
            /**
             * Return boolean value
             */
            return TRUE;
