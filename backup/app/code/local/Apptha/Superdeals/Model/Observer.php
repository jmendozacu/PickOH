...
class Apptha_Superdeals_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    public function getCheckStatus($customArray) {
        ...
        $connection = Mage::getSingleton ( 'core/resource' )->getConnection ( 'core_write' );
        $connection->beginTransaction ();
        $duplicateCollection= Mage::getModel ( 'superdeals/dealz' )->getCollection ()->addFieldToFilter ( 'duplicate', 1 )->addFieldToFilter ( 'order_no', $ordrId );
        $duplicateCollectionCount =  $duplicateCollection->getSize();
        $customUpdateArray = array (
                'status' => $status,
                'smallComplete' => $smallComplete,
                'dupp' => $duplicateCollectionCount,
                'item' => $item,
                'complete' => $Complete,
